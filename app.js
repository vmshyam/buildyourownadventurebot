...
function fDoorOrFind(req, res) {
  ...
  if (sAction.toLowerCase().search("door") != -1){
    twiml.message("**** As you walk through the door, you feel something pulling you into the Cave. You are dragged deep in the dark eerie Cave. You are unable to move and can not feel your body. " +
...
function fWithOrWithGold(req, res) {
  ...
  if (sAction.toLowerCase().search("without") != -1){
    twiml.message("**** As the Cave gets darker, you run out of the Cave as fast as you can. You keep running without stopping, looking for a way out of the Cave. " +
    "You hear eerie noises and see shadow figures consistenly. You suddenly see a door. Do you want to go through the DOOR or keep trying to FIND a way out of the Cave?");
    oConnections[sFrom].fCurState = fDoorOrFind;
...
function fLeftOrRightCave(req, res){
  ...
  if(sAction.toLowerCase().search("left") != -1){
    ...
    "But suddenly the Cave starts to get dark slowing. You want to run out, but should you run out WITH the gold or WITHOUT the gold?");
    oConnections[sFrom].fCurState = fWithOrWithGold;
    ...
    "As you look around and you see a sleeping Dragon. This is the cave of a Dragon. As you nervously panic, you see a wooden bridge crossing over the sleep Dragon to another Cave entrance. You also see a river, flowing away from the Dragon. " +
...
function fSwingOrDash(req, res){
  ...
  if(sAction.toLowerCase().search("swing") != -1){
    ...
    twiml.message("**** You run towards the lantern and accidently slip and knock over the lantern. The oil spills over you and you catch on fire. You try to put the fire out but it is too late. You are burned alive." +
    " NEVER ENTER A UNKNOW CAVE. IT MIGHT BE YOUR LAST!!");
    oConnections[sFrom].fCurState = fEnd;
  }else {
